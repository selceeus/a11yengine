...
class RunContentAuditJob implements ShouldQueue
{
     ...
     * Content violations are pre-computed by the crawler — this job only persists.
     */
    ...
    public function handle(ProcessHtmlScan $processor): void
    {
        ...
        ScanPage::withoutGlobalScopes()
            ...
            ?->update([
                ...
                'visible_text' => $this->visibleText ?: null,
            ]);
    }
    ...
    {
        Log::warning('Content audit job permanently failed', [
            'url' => $this->pageUrl,
            'scan_id' => $this->scan->id,
            'error' => $e->getMessage(),
        ]);
