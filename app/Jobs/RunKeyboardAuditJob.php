...
class RunKeyboardAuditJob implements ShouldQueue
{
    use Batchable, Queueable;

    /**
     * Maximum seconds this job may run before being killed by the queue worker.
     * Keyboard violations are pre-computed by the crawler — this job only persists.
     */
     ...
     * Execute the job.
     *
     * Delegates to ProcessHtmlScan which persists keyboard Findings through
     ...
     * perspective. This ensures keyboard check unavailability never blocks the
     * main scan.
     */
    public function handle(ProcessHtmlScan $processor): void
    {
        try {
            ...
            Log::warning('Keyboard audit failed', [
                'url' => $this->pageUrl,
        ...
        ScanPage::withoutGlobalScopes()
            ...
            ->first()
            ?->update(['keyboard_completed' => true]);
    }
    ...
    {
        Log::warning('Keyboard audit job permanently failed', [
            'url' => $this->pageUrl,
            'scan_id' => $this->scan->id,
            'error' => $e->getMessage(),
        ]);
