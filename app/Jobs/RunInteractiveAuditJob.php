...
class RunInteractiveAuditJob implements ShouldQueue
{
    use Batchable, Queueable;

    /**
     * Maximum seconds this job may run before being killed by the queue worker.
     * Interactive violations are pre-computed by the crawler — this job only persists.
     */
     ...
     * Execute the job.
     *
     * Delegates to ProcessHtmlScan which persists interactive Findings through
     ...
     * perspective. This ensures interactive check unavailability never blocks
     * the main scan.
     */
    public function handle(ProcessHtmlScan $processor): void
    {
        try {
            ...
            Log::warning('Interactive audit failed', [
                'url' => $this->pageUrl,
        ...
        ScanPage::withoutGlobalScopes()
            ...
            ->first()
            ?->update(['interactive_completed' => true]);
    }
    ...
    {
        Log::warning('Interactive audit job permanently failed', [
            'url' => $this->pageUrl,
            'scan_id' => $this->scan->id,
            'error' => $e->getMessage(),
        ]);
